...
@pytest.mark.asyncio
async def test_jwt_required(app):
    url = '/protected'
    ...
    for token in (access_token, fresh_access_token):
        response = await test_client.get(url, headers=make_headers(token))
    ...
    assert await response.get_json() == {'msg': 'Only access tokens are allowed'}


@pytest.mark.asyncio
async def test_fresh_jwt_required(app):
    ...
    response = await test_client.get(url, headers=make_headers(fresh_access_token))
    assert response.status_code == 200
    assert await response.get_json() == {'foo': 'bar'}

    response = await test_client.get(url, headers=make_headers(access_token))
    assert response.status_code == 401
    assert await response.get_json() == {'msg': 'Fresh token required'}

    response = await test_client.get(url, headers=make_headers(fresh_timed_access_token))
    assert response.status_code == 200
    assert await response.get_json() == {'foo': 'bar'}

    response = await test_client.get(url, headers=make_headers(stale_timed_access_token))
    assert response.status_code == 401
    assert await response.get_json() == {'msg': 'Fresh token required'}
    ...
    assert await response.get_json() == {'msg': 'foobar'}
...
async def test_refresh_jwt_required(app):
    ...
    response = await test_client.get(url, headers=make_headers(fresh_access_token))
    assert response.status_code == 422
    assert await response.get_json() == {'msg': 'Only refresh tokens are allowed'}
...
@pytest.mark.asyncio
async def test_jwt_optional(app, delta_func):
    ...
    response = await test_client.get(url, headers=make_headers(fresh_access_token))
    assert response.status_code == 200
    assert await response.get_json() == {'foo': 'baz'}
    ...
    response = await test_client.get(url, headers=make_headers(expired_token))
    assert response.status_code == 401
    assert await response.get_json() == {'msg': 'Token has expired'}


@pytest.mark.asyncio
async def test_invalid_jwt(app):
    url = '/protected'
    jwtM = get_jwt_manager(app)
    test_client = app.test_client()
    ...
    response = await test_client.get(url, headers=make_headers(invalid_token))
    assert response.status_code == 422
    assert await response.get_json() == {'msg': 'Invalid header padding'}
...
@pytest.mark.asyncio
async def test_jwt_missing_claims(app):
    url = '/protected'
    test_client = app.test_client()
    token = await encode_token(app, {'foo': 'bar'})

    response = await test_client.get(url, headers=make_headers(token))
    assert response.status_code == 422
    assert await response.get_json() == {'msg': 'Missing claim: identity'}


@pytest.mark.asyncio
async def test_jwt_invalid_audience(app):
    url = '/protected'
    test_client = app.test_client()
    ...
    response = await test_client.get(url, headers=make_headers(access_token))
    assert response.status_code == 422
    assert await response.get_json() == {'msg': 'Token is missing the "aud" claim'}

    # Audience claim still expected and wrong one provided - not OK
    access_token = await encode_token(app, {'aud': 'different_audience', 'identity': 'me'})
    response = await test_client.get(url, headers=make_headers(access_token))
    assert response.status_code == 422
    assert await response.get_json() == {'msg': 'Invalid audience'}
...
async def test_jwt_invalid_issuer(app):
    ...
    response = await test_client.get(url, headers=make_headers(access_token))
    assert response.status_code == 422
    assert await response.get_json() == {'msg': 'Token is missing the "iss" claim'}

    # Issuer claim still expected and wrong one provided - not OK
    access_token = await encode_token(app, {'iss': 'different_issuer', 'identity': 'me'})
    response = await test_client.get(url, headers=make_headers(access_token))
    assert response.status_code == 422
...
async def test_malformed_token(app):
    ...
    response = await test_client.get(url, headers=make_headers(access_token))
    assert response.status_code == 422
    assert await response.get_json() == {'msg': 'Not enough segments'}
...
@pytest.mark.asyncio
async def test_expired_token_via_decode_token(app):
    jwtM = get_jwt_manager(app)
...
@pytest.mark.asyncio
async def test_different_token_algorightm(app):
    url = '/protected'
    test_client = app.test_client()
    async with app.test_request_context("/protected"):
    ...
    response = await test_client.get(url, headers=make_headers(token))
    assert response.status_code == 422
    assert await response.get_json() == {'msg': 'The specified alg value is not allowed'}
