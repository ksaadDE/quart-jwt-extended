...
@pytest.mark.asyncio
async def test_jwt_refresh_required_with_cookies(app, options):
    test_client = app.test_client()
    auth_url, cookie_name, protected_url, delete_url = options

    # Test without cookies
    response = await test_client.get(protected_url)
    assert response.status_code == 401
    assert await response.get_json() == {'msg': 'Missing cookie "{}"'.format(cookie_name)}

    # Test after receiving cookies
    await test_client.get(auth_url)
    response = await test_client.get(protected_url)
    assert response.status_code == 200
    assert await response.get_json() == {'foo': 'bar'}

    # Test after issuing a 'logout' to delete the cookies
    await test_client.get(delete_url)
    response = await test_client.get(protected_url)
    assert response.status_code == 401
    assert await response.get_json() == {'msg': 'Missing cookie "{}"'.format(cookie_name)}
    ...
    await test_client.get("/delete_tokens")
    response = await test_client.get(protected_url)
    assert response.status_code == 401
    assert await response.get_json() == {'msg': 'Missing cookie "{}"'.format(cookie_name)}
...
@pytest.mark.asyncio
async def test_default_access_csrf_protection(app, options):
    test_client = app.test_client()
    auth_url, csrf_cookie_name, post_url = options

    # Get the jwt cookies and csrf double submit tokens
    response = await test_client.get(auth_url)
    ...
    response = await test_client.post(post_url)
    assert response.status_code == 401
    assert await response.get_json() == {'msg': 'Missing CSRF token'}

    # Test that you can post with the csrf double submit value
    csrf_headers = {'X-CSRF-TOKEN': csrf_token}
    response = await test_client.post(post_url, headers=csrf_headers)
    assert response.status_code == 200
    assert await response.get_json() == {'foo': 'bar'}
...
@pytest.mark.asyncio
async def test_non_matching_csrf_token(app, options):
    test_client = app.test_client()
    ...
    response = await test_client.post(post_url, headers=csrf_headers)
    assert response.status_code == 401
    assert await response.get_json() == {'msg': 'CSRF double submit tokens do not match'}
...
@pytest.mark.asyncio
async def test_csrf_disabled(app, options):
...
@pytest.mark.asyncio
async def test_csrf_with_custom_header_names(app, options):
...
@pytest.mark.asyncio
async def test_csrf_with_default_form_field(app, options):
    ...
    response = await test_client.post(post_url, form=csrf_data)
    assert response.status_code == 200
    assert await response.get_json() == {'foo': 'bar'}
...
@pytest.mark.asyncio
async def test_csrf_with_custom_form_field(app, options):
    app.config['JWT_CSRF_CHECK_FORM'] = True
...
@pytest.mark.asyncio
async def test_custom_csrf_methods(app, options):
    ...
    response = await test_client.get(get_url)
    assert response.status_code == 401
    assert await response.get_json() == {'msg': 'Missing CSRF token'}

    # Insure GET requests now succeed with csrf
    csrf_headers = {'X-CSRF-TOKEN': csrf_token}
    response = await test_client.get(get_url, headers=csrf_headers)
    assert response.status_code == 200
    assert await response.get_json() == {'foo': 'bar'}


@pytest.mark.asyncio
async def test_setting_cookies_wihout_cookies_enabled(app):
    app.config['JWT_TOKEN_LOCATION'] = ['headers']
    test_client = app.test_client()

    response = await test_client.get('/access_token')
    assert response.status_code == 500
    response = await test_client.get('/refresh_token')
    assert response.status_code == 500
    response = await test_client.get('/delete_tokens')
    assert response.status_code == 500
    response = await test_client.get('/delete_access_tokens')
    assert response.status_code == 500
    response = await test_client.get('/delete_refresh_tokens')
    assert response.status_code == 500


@pytest.mark.asyncio
async def test_default_cookie_options(app):
    test_client = app.test_client()
...
@pytest.mark.asyncio
async def test_custom_cookie_options(app):
    test_client = app.test_client()
...
@pytest.mark.asyncio
async def test_custom_cookie_names_and_paths(app):
    test_client = app.test_client()
...
@pytest.mark.asyncio
async def test_csrf_token_not_in_cookie(app):
    test_client = app.test_client()
...
@pytest.mark.asyncio
async def test_cookies_without_csrf(app):
    test_client = app.test_client()
...
@pytest.mark.asyncio
async def test_jwt_optional_with_csrf_enabled(app):
    test_client = app.test_client()

    # User without a token should be able to reach the endpoint without
    # getting a CSRF error
    response = await test_client.post('/optional_post_protected')
    assert response.status_code == 200
    assert await response.get_json() == {'foo': 'bar'}
