...
:meth:`~quart_jwt_extended.JWTManager.user_loader_callback_loader` decorator. The
resulting object can be accessed in your protected endpoints by using the
:func:`~quart_jwt_extended.get_current_user` function, or directly with
the :attr:`~quart_jwt_extended.current_user` LocalProxy.

One thing to note is if you access a database in the
:meth:`~quart_jwt_extended.JWTManager.user_loader_callback_loader`, you will incur
